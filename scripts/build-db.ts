#!/usr/bin/env tsx
/**
 * Database builder for Venezuela Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Venezuela Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Venezuelan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'VE');
    insertMeta.run('source', 'venezuela.justia.com');
